...
function generateTargetsConfig($db): string {
    $c = "# Auto-generated by SmokePing Manager v3\n# " . date('Y-m-d H:i:s') . "\n\n";
    $c .= "*** Targets ***\n\nprobe = FPing\nmenu = Top\ntitle = Netwerk Latency Monitor\nremark = Beheerd via SmokePing Manager\n\n";
...
if ($_SERVER['REQUEST_METHOD']==='POST' && $act) {
    ...
    if ($act==='backup') {
        ...
        flash("Backup gemaakt: backup_$stamp"); redir('backup');
    }
    if ($act==='restore') {
...
<style>
:root{--bg:#0c0e14;--s1:#151821;--s2:#1c2030;--s3:#252a3a;--brd:#2a3045;--tx:#dde1ed;--txd:#7b82a0;--ac:#4f8ff7;--ach:#6ba0f9;--ok:#34d399;--err:#f87171;--warn:#fbbf24;--v6:#a78bfa;--r:8px}
...
.hd{background:var(--s1);border-bottom:1px solid var(--brd);padding:0 20px;height:52px;display:flex;align-items:center;justify-content:space-between}
...
.nv a:hover,.nv button:hover{background:var(--s2);color:var(--tx)}.nv .on{background:var(--s2);color:var(--tx)}
...
.fl.success{background:#0a2618;border-color:#145230;color:var(--ok)}.fl.error{background:#2a0f0f;border-color:#5f1a1a;color:var(--err)}
.cd{background:var(--s1);border:1px solid var(--brd);border-radius:var(--r);padding:20px;margin-bottom:16px}
...
.bg{background:0;color:var(--txd);border:1px solid var(--brd)}.bg:hover{background:var(--s2);color:var(--tx)}
...
.bge-on{background:#0a2618;color:var(--ok)}.bge-off{background:#2a0f0f;color:var(--err)}
.bge-p{background:#162040;color:var(--ac)}.bge-v6{background:#1f1540;color:var(--v6)}
.bge-down{background:#451a03;color:var(--warn)}
.lw{min-height:100vh;display:flex;align-items:center;justify-content:center}
.lb{width:340px;background:var(--s1);border:1px solid var(--brd);border-radius:10px;padding:28px}
...
.mo.on{display:flex}
.md{background:var(--s1);border:1px solid var(--brd);border-radius:10px;padding:22px;width:500px;max-width:92vw;max-height:90vh;overflow-y:auto}
...
.sc{background:var(--s2);border-radius:var(--r);padding:14px;text-align:center}
.sc .n{font-size:24px;font-weight:700;color:var(--ac)}.sc .l{font-size:11px;color:var(--txd);margin-top:3px}
.cc{background:var(--s1);border:1px solid var(--brd);border-radius:var(--r);padding:14px;margin-bottom:10px;display:flex;justify-content:space-between;align-items:center;transition:.15s}
...
.pg{background:var(--s2);border-radius:var(--r);padding:14px;margin-bottom:10px}
...
.down-info{font-size:11px;color:var(--warn);background:#1a1000;padding:6px 10px;border-radius:var(--r);margin-top:6px;font-family:monospace;white-space:pre-wrap}
...
?>
<h2 style="font-size:16px;margin-bottom:14px">Overzicht — Alle Targets</h2>
...
<button type="submit" class="bt bp">📦 Nieuwe Backup Maken</button>
<span style="font-size:12px;color:var(--txd);margin-left:10px">Slaat config, database en RRD data op</span></form></div>
<?php if(!empty($backups)):?><div class="cd"><div class="cd-t">Beschikbare Backups</div>
<table class="tb"><thead><tr><th>Naam</th><th style="text-align:right">Acties</th></tr></thead><tbody>
<?php foreach($backups as $b):?><tr><td><strong><?=e($b)?></strong></td><td style="text-align:right">
<form method="POST" style="display:inline" onsubmit="return confirm('Backup terugzetten? Dit overschrijft huidige config!')"><input type="hidden" name="action" value="restore"><?=csrfField()?><input type="hidden" name="backup_name" value="<?=e($b)?>"><button class="bt bs bsm">Terugzetten</button></form>
