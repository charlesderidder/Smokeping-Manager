#!/bin/bash
# ============================================================
# SmokePing Manager 6.3 - Complete All-in-One Installer
#
# Features:
...
#   - 6.3 - Bugfix: dubbele CustomGraphGroup categorieen in Targets bestand (samengestelde grafieken) opgelost.
#
...
show_menu() {
    clear
    echo -e "${ORANGE}╔══════════════════════════════════════════════════════════════╗${NC}"
    echo -e "${ORANGE}║${NC}            ${CYAN}SmokePing Manager 6.3 - Installer${NC}                 ${ORANGE}║${NC}"
...
EOF
    ...
    echo ""
    echo -e "${ORANGE}════════════════════════════════════════════${NC}"
    echo -e "${GREEN} ✓ SmokePing Manager 6.3 geïnstalleerd!${NC}"
    echo -e "${ORANGE}════════════════════════════════════════════${NC}"
...
function generateTargetsConfig($db): string {
    $c = "# Auto-generated by SmokePing Manager v6.3\n# " . date('Y-m-d H:i:s') . "\n\n";
    $c .= "*** Targets ***\n\nprobe = FPing\nmenu = Top\ntitle = Netwerk Latency Monitor\nremark = Beheerd via SmokePing Manager\n\n";
    ...
    if (!empty($customGraphs)) {
        ...
        // Keep track of which category headers we've emitted to avoid duplicates
        $seenCategoryHeaders = [];
        foreach ($groupedGraphs as $groupKey => $groupData) {
            $groupLabel = str_replace(["\r","\n","\t"], ' ', (string)$groupData['label']);

            // If this category already exists (from regular categories or IPv6 auto-generated categories),
            // do NOT emit a second '+ Category' header. Instead, append graph entries under the existing category.
            if (!isset($existingCategoryNames[$groupKey]) && !isset($seenCategoryHeaders[$groupKey])) {
                $c .= "+ {$groupKey}\nmenu = {$groupLabel}\ntitle = {$groupLabel}\n\n";
                $seenCategoryHeaders[$groupKey] = $groupLabel;
                // Mark as existing to prevent future duplicates
                $existingCategoryNames[$groupKey] = true;
            } else {
                // Ensure separation before appending graph entries
                $c .= "\n";
            }
